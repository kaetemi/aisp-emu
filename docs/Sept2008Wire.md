...
`recv_get_item_base_list_r` `0xC7A9` is a result uint and a count (`≤ 0x7D0`). Count 0 is 8 bytes and clears that post-enter wait. `0xCD38` time-of-day is four uints; the later flag byte fails the consume check. After those, the client asks for friend link tags (`0x0F97`). The 24-byte `0x239E` reply still closes the area socket.
