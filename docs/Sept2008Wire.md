...
`recv_notify_change_map` on this exe is opcode **`0xB315`**. The area switch compares `0xB235` and takes the greater-than arm (`0x6a87a7`), then subtracts `0xB2A9`, `0x6B`, and `1`. The fall-through (`0x6a883c`, alloc `0x68`, slot `0x0a`) is `0xB315`. The body is the 98-byte July 2009 layout: 30-byte route (`0x696980`: four uints, XYZ, rotation, animation), fade byte (`0x695f50`), port uint16 and 65-byte IP (`0x6967d0`). The `0x20` and `0x44` helpers reserve space in that alloc; they do not add those sizes to the packet. `0xB235` (`0x6a8663`, slot `0x32`) is a different packet: two uints, 8 bytes. A 98-byte body on `0xB235` fails the consume check and resets Area. 2011 reused `0xB235` as `recv_robo_rest_r`. Callback `0x6907d0` (`vtable+0x84`). An empty IP leaves the string size at 0, and `0x61bb40` skips the area reconnect. Live walk, 2026-09-21: crossing the Sotokanda cubes sent `NotifyChangeMap` opcode `0xB315`, 98 bytes, host empty and port 0. Area stayed connected. The client then requested map links for `10990300` and sent `MapEnterRequest` for that map. Area version-check is extra 2, crc `0x9E57B1E4` (`0x693792`). `recv_notify_maplink_data` `0x5755` is present (25 bytes, same as the 2011 reader). `recv_notify_select_map` `0x68A5` is absent.
...
`recv_notify_avatar_data` `0x7D78` (parser `0x6a3f70`) is result, the selected avatar id, then a short record: id, model, 37-byte name, 19-byte visual, one uint, the 30-byte map, 29 equip pairs, five 16-byte blocks, and two uints (426 bytes). The id must be the one stored at select (`0x89de00`). The 2009/2011 notify is longer and fails the consume check.

`recv_get_item_base_list_r` `0xC7A9` is a result uint and a count (`≤ 0x7D0`). Count 0 is 8 bytes and clears that post-enter wait. `0xCD38` time-of-day is four uints; the later flag byte fails the consume check. After those, the client asks for friend link tags (`0x0F97`) and waits in scene state `0xFD2`. `recv_get_friend_link_tag_data_r` `0x239E` (parser `0x69b173`, alloc `0x16C`) is result, avatar id, a tag count (`≤ 5`) of uint-plus-61-byte records, and a slot count (`≤ 5`) of uints. Both counts 0 is 16 bytes. Result 0 moves `0xFD2` → `0xFDC`. The 2009/2011 questionnaire lists are extra and fail the consume check. The empty friend list (`0x2411`, 16 bytes), empty mailbox (`0x147A`, 8 bytes), and circle reply are then accepted, and the client shows Now Loading. `recv_notify_monster_data` `0xCD67` (parser `0x6ab4d8`, slot `0x18`) is result, object id, and the short character record (`0x696e20`, 338 bytes). The 555-byte 2009/2011 notify fails that consume check and closes area. The solicited `0xCD38` reply is those four uints. Map enter then completes and the doll is in the plaza.
