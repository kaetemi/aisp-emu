...
- `CAipowerWindow` (IF `0x4B`, ctor `0x5d3820`, alloc `0xF40`) is **AIパワー**, a charadoll support/cheer sheet, not a fight HUD. PAS `ai_power00.xml` titles it `aiパワー`. Tabs are 応援画面 and 要望箱. Each card has a **キャラクターヴィジュアル** (256×256 DXT5 portrait from `aipower.hed`, not a 3D doll), profile (height / weight / three-size / likes / dislikes), a dialogue balloon, two gauges, and **six heart pips** (`ハート-1`…`ハート-6` plus `ハート-max`). Buttons spend AI points or nico points (`応援する！`, `一口応援`). The texture atlas is stamped **開発中！**. 2009 help never mentions it; it is not on ウィンドウメニュー.
- The window opens when Area sends `recv_aipower_data` (`0x59C3`, handler `0x72ABA0`): 8-byte header (`result` then `count`) + `N` records of `0x4C0` bytes, `N ≤ 0x12C` (300). After parse it calls vtable `+0x518`. Internal UI message `0x30001` can also construct type `0x4B` locally. `recv_close_aipower_window_r` (`0x6B53`) is a 4-byte result (vtable `+0x524`). `recv_support_aipower_aipoint_r` (`0x5DB8`) is 12 bytes. 2011 deleted `CAipowerWindow` RTTI but kept the PAS/DDS and VCE log stubs; TPS `tps_status00.xml` (`TPS戦闘-HPステータス`) is the 2011 heart/stamina HUD. `/aipower` on this branch pushes that notify (optional count 0–300; `empty` for a header-only dump).
- Card apply is `CAipowerCharaSheet` `0x5d2320`. `Id` is **not** an avatar id: `str_table` `(100,940,1)` is `./aipower/%05d.dds`. A miss hides the sheet (that is why a card with `Id=1` opened an empty overlay). July 2009 `aipower.hed` (key `6E3C`) holds three DXT5 256×256 portraits: `10000` 月島小恋, `10001` 白河ななか, `10002` 朝倉由夢. `Param0` / `Param1` are 0–100 gauge percents (`0x5d1920` / `0x5d1840`); heart pip = PAS `380 + Param0/20`, and `Param0 ≥ 100` shows MAX. `Param2` is added into the upper gauge when a cheer tick overflows the lower one. Profile strings (PAS 190–194) are height / weight / three-size / likes / dislikes. The 601-byte tail is a UTF-8 C string (MultiByteToWideChar 65001); `<BR>` splits balloon lines (wide needle at `0x8a4810`).
